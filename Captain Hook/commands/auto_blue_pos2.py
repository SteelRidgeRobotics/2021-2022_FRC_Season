...
class AutoBluePos2(commands2.SequentialCommandGroup):
    ...
    def __init__(self, intake: Intake, launcher: Launcher, drivetrain: Drivetrain, pathTrajectory: PathTrajectory) -> None:
        ...
        self.addCommands(
            InstantCommand(lambda: drivetrain.resetOdometry(pathTrajectory.trajectoryBlue1p1.initialPose())),
            ParallelCommandGroup(LaunchCargo(launcher),ToggleIntakePosition(intake)),
            ParallelCommandGroup(drivetrain.createTrajectoryCommand(pathTrajectory.trajectoryBlue1p1, False).withTimeout(50),TimedIntakeBottom(intake, 0.30, 4.0)),
            ToggleIntakePosition(intake),
            WaitCommand(3.0),
            TimedIntakeTop(intake, -0.25, 2.0),
            drivetrain.createTrajectoryCommand(pathTrajectory.trajectoryBlue1p2, False).withTimeout(50),
